...
function generateJvmArgs(allocMb,profile,aikar,disableAnim,reduceView){let args=['-Xmx'+allocMb+'M','-Xms'+Math.floor(allocMb*0.5)+'M'];if(aikar){args.push('-XX:+UseG1GC','-XX:+ParallelRefProcEnabled','-XX:MaxGCPauseMillis=200','-XX:+UnlockExperimentalVMOptions','-XX:G1NewCollectionHotspotThreshold=2','-XX:G1RSetUpdatingPauseTimePercent=5','-XX:SurvivorRatio=32','-XX:+PerfDisableSharedMem','-XX:G1HeapRegionSize=16M')}if(profile==='low'){args.push('-XX:+UseStringDeduplication','-XX:-UseAdaptiveSizePolicy','-XX:G1HeapWastePercent=10')}else if(profile==='medium'){args.push('-XX:+UseStringDeduplication')}return args.join(' ')}
...
function loadSavedOptimization(){try{apiFetch('/api/settings/config').then(r=>{if(!r.ok)throw new Error('Failed to load config');return r.json()}).then(j=>{const jvmArgs=j.jvmArgs||[];console.log('Loaded config:',{jvmArgs});const rec=getRAMRecommendation(deviceSpecs);if(el('ramSlider')){el('ramSlider').max=rec.max;el('ramSlider').min=256}let allocatedRam=rec.recommended;if(jvmArgs&&jvmArgs.length>0){const argsStr=Array.isArray(jvmArgs)?jvmArgs.join(' '):jvmArgs;const xmxMatch=argsStr.match(/-Xmx(\d+)M/);if(xmxMatch){allocatedRam=parseInt(xmxMatch[1])}console.log('Extracted allocated RAM from JVM args:',allocatedRam);el('allocatedRam').value=allocatedRam;el('ramSlider').value=allocatedRam;const hasAikar=argsStr.includes('UseG1GC');const hasDisableAnim=argsStr.includes('-XX:-UseAdaptiveSizePolicy');if(el('optAikarFlags')){el('optAikarFlags').checked=hasAikar}if(el('optDisableAnimations')){el('optDisableAnimations').checked=hasDisableAnim}detectProfileFromJvmArgs(argsStr);const profile=el('optimizationProfile').value;if(profile==='low'){el('optReduceViewDist').checked=true}else{el('optReduceViewDist').checked=false}}else{el('allocatedRam').value=rec.recommended;el('ramSlider').value=rec.recommended;el('optimizationProfile').value='medium';el('optAikarFlags').checked=true;el('optDisableAnimations').checked=false;el('optReduceViewDist').checked=false}updateRAMSlider();updateOptimizedArgs()}).catch(e=>{console.warn('Failed to load saved optimization',e);const rec=getRAMRecommendation(deviceSpecs);if(el('ramSlider')){el('ramSlider').max=rec.max;el('ramSlider').min=256}el('allocatedRam').value=rec.recommended;el('ramSlider').value=rec.recommended;el('optimizationProfile').value='medium';el('optAikarFlags').checked=true;el('optDisableAnimations').checked=false;el('optReduceViewDist').checked=false;updateRAMSlider();updateOptimizedArgs()})}catch(e){console.warn('Load saved optimization error',e)}}
...
function updateOptimizedArgs(){const allocMb=parseInt(el('allocatedRam').value)||2048;const profile=el('optimizationProfile').value;const aikar=el('optAikarFlags').checked;const disableAnim=el('optDisableAnimations').checked;const reduceView=el('optReduceViewDist').checked;const args=generateJvmArgs(allocMb,profile,aikar,disableAnim,reduceView);el('optimizedJvmArgs').value=args}
if(el('optimizationProfile')){el('optimizationProfile').addEventListener('change',()=>{const profile=el('optimizationProfile').value;if(profile==='low'){el('allocatedRam').value=1024;el('optAikarFlags').checked=true;el('optDisableAnimations').checked=true;el('optReduceViewDist').checked=true}else if(profile==='medium'){el('allocatedRam').value=Math.min(4096,getRAMRecommendation(deviceSpecs).recommended);el('optAikarFlags').checked=true;el('optDisableAnimations').checked=false;el('optReduceViewDist').checked=false}else if(profile==='high'){el('allocatedRam').value=getRAMRecommendation(deviceSpecs).max;el('optAikarFlags').checked=true;el('optDisableAnimations').checked=false;el('optReduceViewDist').checked=false}const slider=el('ramSlider');if(slider){slider.value=el('allocatedRam').value;el('ramPercentage').textContent=el('allocatedRam').value+' MB'}updateOptimizedArgs()})}
...
if(el('optDisableAnimations')){el('optDisableAnimations').addEventListener('change',()=>{updateOptimizedArgs()})}
if(el('optReduceViewDist')){el('optReduceViewDist').addEventListener('change',()=>{updateOptimizedArgs()})}
...
if(el('applyOptimization')){el('applyOptimization').onclick=async()=>{try{const args=el('optimizedJvmArgs').value;const jvmArgsArray=args.split(/\s+/).filter(Boolean);console.log('Applying optimization with JVM args:',jvmArgsArray);const r=await apiFetch('/api/settings/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jvmArgs:jvmArgsArray})});if(r.ok){showToast('Optimization applied! Restart server to take effect.','success',6000);setTimeout(()=>{loadSavedOptimization()},500)}else{const errText=await r.text();console.error('API error:',r.status,errText);showToast('Failed to apply optimization: '+r.status,'danger',6000)}}catch(e){console.error('Error applying optimization:',e);showToast('Error applying optimization: '+e.message,'danger',6000)}}}
if(el('resetOptimization')){el('resetOptimization').onclick=async()=>{try{const rec=getRAMRecommendation(deviceSpecs);el('allocatedRam').value=rec.recommended;el('optimizationProfile').value='medium';el('optAikarFlags').checked=true;el('optDisableAnimations').checked=false;el('optReduceViewDist').checked=false;updateRAMSlider();updateOptimizedArgs();showToast('Reset to recommended defaults','info',4000)}catch(e){showToast('Error resetting optimization','danger',6000)}}}
...
async function loadFiles(p){
  ...
  if(!e.isDir && (e.name.toLowerCase().endsWith('.zip')||e.name.toLowerCase().endsWith('.rar'))){const uz=document.createElement('button');uz.className='btn-file';uz.textContent='Unzip';uz.onclick=async()=>{const ok=confirm('Extract this archive here?\n\nThis may take a while for large files.');if(!ok)return;const body={path:pathJoin(p,e.name),dest:p,inst:activeInstance};const box=el('fileUploadProgress');const bar=el('fileUploadBar');const txt=el('fileUploadText');let prog=0;let timer=null;try{if(box){box.classList.remove('hidden');bar&&bar.style.setProperty('--w','0%');if(txt)txt.textContent='Extracting...';try{box.scrollIntoView({ behavior:'smooth', block:'start' })}catch{};timer=setInterval(()=>{prog=Math.min(95,prog+1);if(bar)bar.style.setProperty('--w',prog+'%')},300)}uz.disabled=true;let r=await apiFetch('/api/fs/unzip',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!r.ok){r=await apiFetch(urlInst('/api/fs/unzip?path='+encodeURIComponent(body.path)+'&dest='+encodeURIComponent(body.dest)))}if(r.ok){const j=await r.json();if(bar)bar.style.setProperty('--w','100%');if(txt)txt.textContent='100%';showToast('Archive extracted successfully!','success',6000);setTimeout(()=>{if(box)box.classList.add('hidden');loadFiles(p)},1000)}else{let errorMsg='Extract failed';try{const errJson=await r.json();if(errJson.details)errorMsg+=': '+errJson.details;else if(errJson.error)errorMsg+=': '+errJson.error}catch{}showToast(errorMsg,'danger',10000);if(box)box.classList.add('hidden')}}finally{if(timer)clearInterval(timer);uz.disabled=false}};tdA.appendChild(uz)}
...
function openEditor(fullPath){el('editorPath').textContent=fullPath;document.body.classList.add('modal-open');el('editor').classList.remove('hidden');apiFetch(urlInst('/api/fs/read?path='+encodeURIComponent(fullPath))).then(r=>{if(!r.ok)throw new Error('Failed to read file');return r.json()}).then(j=>{el('editorContent').value=j.content}).catch(e=>{el('editorContent').value='Error reading file: '+e.message;el('editorContent').disabled=true})}
