...
el('uploadBtn').onclick=async()=>{const files=el('uploadFile').files;if(!files||files.length===0)return;const p=el('pathInput').value;const box=el('fileUploadProgress');const bar=el('fileUploadBar');const txt=el('fileUploadText');box.classList.remove('hidden');bar.style.setProperty('--w','0%');txt.textContent='0%';el('uploadBtn').disabled=true;el('uploadFile').disabled=true;try{const arr=[...files];const totalSize=arr.reduce((a,f)=>a+f.size,0);const hasBigZip=arr.some(f=>f.name.toLowerCase().endsWith('.zip')&&f.size>4*1024*1024);let uploaded=0;const update=(bytes)=>{uploaded+=bytes;const pct=Math.min(100,Math.round(uploaded*100/Math.max(1,totalSize)));bar.style.setProperty('--w',pct+'%');txt.textContent=pct+'%'};let ok=false;if(!hasBigZip){try{const fd=new FormData();for(const f of arr){fd.append('files',f)}fd.append('dest',p);fd.append('inst',activeInstance||'');const r=await apiUpload(urlInst('/api/fs/upload-multi'),fd,(pct)=>{bar.style.setProperty('--w',pct+'%');txt.textContent=pct+'%'},{ stallMs: 20000, timeoutMs: 10*60*1000 });ok=r.ok}catch{}}if(!ok&&!hasBigZip){try{for(const f of arr){const fd1=new FormData();fd1.append('file',f);fd1.append('dest',p);fd1.append('inst',activeInstance||'');const r1=await apiUpload(urlInst('/api/fs/upload'),fd1,(pct)=>{update(Math.round(f.size*pct/100))},{ stallMs: 20000, timeoutMs: 10*60*1000 });if(!(r1&&r1.ok)) throw new Error('single_fail')}ok=true}catch{}}if(!ok){for(const f of arr){await uploadChunked(f,p,(bytes)=>update(bytes))}ok=true}el('uploadFile').value='';loadFiles(p)}catch(e){alert('Upload failed; try smaller files or upload one-by-one')}finally{el('uploadBtn').disabled=false;el('uploadFile').disabled=false;bar.style.setProperty('--w','0%');txt.textContent='';box.classList.add('hidden')}}
...
el('pluginUpload').onclick=async()=>{const files=el('pluginFile').files;if(!files||files.length===0)return;const fd=new FormData();for(const f of files){fd.append('files',f)}fd.append('inst',activeInstance||'');const box=el('pluginUploadProgress');const bar=el('pluginUploadBar');const txt=el('pluginUploadText');box.classList.remove('hidden');bar.style.setProperty('--w','0%');txt.textContent='0%';el('pluginUpload').disabled=true;el('pluginFile').disabled=true;try{let r=await apiUpload(urlInst('/api/plugins/upload-multi'),fd,(pct)=>{bar.style.setProperty('--w',pct+'%');txt.textContent=pct+'%'});if(!r.ok){for(const f of files){const fd1=new FormData();fd1.append('file',f);fd1.append('inst',activeInstance||'');const r1=await apiUpload(urlInst('/api/plugins/upload'),fd1);if(!r1.ok)throw new Error('upload_failed')}}el('pluginFile').value='';loadPlugins()}catch(e){alert('Plugin upload failed; try single-file uploads')}finally{el('pluginUpload').disabled=false;el('pluginFile').disabled=false;bar.style.setProperty('--w','0%');txt.textContent='';box.classList.add('hidden')}}
...
async function uploadChunked(file,dest,onBytes){const start=await apiFetch('/api/fs/upload-chunk/start',{method:'POST'});const s=await start.json();let offset=0;const id=s.uploadId;const size=file.size;const chunkSize=1024*1024;while(offset<size){const end=Math.min(offset+chunkSize,size);const chunk=file.slice(offset,end);const r=await apiUploadRaw(`/api/fs/upload-chunk/append-raw?uploadId=${encodeURIComponent(id)}&offset=${offset}`,chunk,null,{ stallMs: 20000, timeoutMs: 10*60*1000 });if(!r.ok) throw new Error('chunk_failed');const len=end-offset;if(onBytes)onBytes(len);try{const jr=JSON.parse(r.body||'{}');offset=jr.nextOffset||end}catch{offset=end}}const fin=await apiFetch('/api/fs/upload-chunk/finish',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uploadId:id,dest:dest,name:file.name,inst:activeInstance})});if(!fin.ok) throw new Error('finish_failed')}
