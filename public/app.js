...
function generateJvmArgs(allocMb,profile,aikar){let args=['-Xmx'+allocMb+'M','-Xms'+Math.floor(allocMb*0.5)+'M'];if(aikar){args.push('-XX:+UseG1GC','-XX:+ParallelRefProcEnabled','-XX:MaxGCPauseMillis=200','-XX:+UnlockExperimentalVMOptions','-XX:G1NewCollectionHotspotThreshold=2','-XX:G1RSetUpdatingPauseTimePercent=5','-XX:SurvivorRatio=32','-XX:+PerfDisableSharedMem','-XX:G1HeapRegionSize=16M')}if(profile==='low'){args.push('-XX:+UseStringDeduplication','-XX:-UseAdaptiveSizePolicy','-XX:G1HeapWastePercent=10')}else if(profile==='medium'){args.push('-XX:+UseStringDeduplication')}else if(profile==='custom'){args.push('-XX:+UseCustomProfile')}return args.join(' ')}
