...
el('uploadBtn').onclick=async()=>{const files=el('uploadFile').files;if(!files||files.length===0)return;const p=el('pathInput').value;const fd=new FormData();for(const f of files){fd.append('files',f)}fd.append('dest',p);fd.append('inst',activeInstance||'');const box=el('fileUploadProgress');const bar=el('fileUploadBar');const txt=el('fileUploadText');box.classList.remove('hidden');bar.style.setProperty('--w','0%');txt.textContent='0%';el('uploadBtn').disabled=true;el('uploadFile').disabled=true;try{let ok=false;try{const r=await apiUpload(urlInst('/api/fs/upload-multi'),fd,(pct)=>{bar.style.setProperty('--w',pct+'%');txt.textContent=pct+'%'});ok=r.ok}catch{}if(!ok){try{for(const f of files){const fd1=new FormData();fd1.append('file',f);fd1.append('dest',p);fd1.append('inst',activeInstance||'');const r1=await apiUpload(urlInst('/api/fs/upload'),fd1);if(!(r1&&r1.ok)) throw new Error('single_fail')}ok=true}catch{} }if(!ok){for(const f of files){await uploadChunked(f,p)}ok=true}el('uploadFile').value='';loadFiles(p)}catch(e){alert('Upload failed; try smaller files or upload one-by-one')}finally{el('uploadBtn').disabled=false;el('uploadFile').disabled=false;bar.style.setProperty('--w','0%');txt.textContent='';box.classList.add('hidden')}}
...
async function uploadChunked(file,dest){const start=await apiFetch('/api/fs/upload-chunk/start',{method:'POST'});const s=await start.json();let offset=0;const id=s.uploadId;const size=file.size;const chunkSize=512*1024;while(offset<size){const end=Math.min(offset+chunkSize,size);const chunk=file.slice(offset,end);const fd=new FormData();fd.append('chunk',chunk);fd.append('uploadId',id);fd.append('offset',String(offset));const r=await apiUpload('/api/fs/upload-chunk/append',fd,(pct)=>{});if(!r.ok) throw new Error('chunk_failed');try{const jr=JSON.parse(r.body||'{}');offset=jr.nextOffset||end}catch{offset=end}}const fin=await apiFetch('/api/fs/upload-chunk/finish',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uploadId:id,dest:dest,name:file.name,inst:activeInstance})});if(!fin.ok) throw new Error('finish_failed')}
