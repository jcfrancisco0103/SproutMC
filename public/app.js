...
el('pluginUpload').onclick=async()=>{const files=el('pluginFile').files;if(!files||files.length===0)return;const box=el('pluginUploadProgress');const bar=el('pluginUploadBar');const txt=el('pluginUploadText');box.classList.remove('hidden');bar.style.setProperty('--w','0%');txt.textContent='0%';el('pluginUpload').disabled=true;el('pluginFile').disabled=true;try{const arr=[...files];const totalSize=arr.reduce((a,f)=>a+f.size,0);const hasLarge=arr.some(f=>f.size>2*1024*1024||/\.(zip|rar|jar)$/i.test(f.name));let uploaded=0;const update=(bytes)=>{uploaded+=bytes;const pct=Math.min(100,Math.round(uploaded*100/Math.max(1,totalSize)));bar.style.setProperty('--w',pct+'%');txt.textContent=pct+'%'};let ok=false;if(!hasLarge){try{const fd=new FormData();for(const f of arr){fd.append('files',f)}fd.append('inst',activeInstance||'');const r=await apiUpload(urlInst('/api/plugins/upload-multi'),fd,(pct)=>{bar.style.setProperty('--w',pct+'%');txt.textContent=pct+'%'},{ stallMs: 20000, timeoutMs: 10*60*1000 });ok=r.ok}catch{}}if(!ok&&!hasLarge){try{for(const f of arr){const fd1=new FormData();fd1.append('file',f);fd1.append('inst',activeInstance||'');const r1=await apiUpload(urlInst('/api/plugins/upload'),fd1,(pct)=>{update(Math.round(f.size*pct/100))},{ stallMs: 20000, timeoutMs: 10*60*1000 });if(!(r1&&r1.ok)) throw new Error('single_fail')}ok=true}catch{}}if(!ok){for(const f of arr){await uploadChunked(f,'plugins',(bytes)=>update(bytes))}ok=true}el('pluginFile').value='';loadPlugins()}catch(e){alert('Plugin upload failed')}finally{el('pluginUpload').disabled=false;el('pluginFile').disabled=false;bar.style.setProperty('--w','0%');txt.textContent='';box.classList.add('hidden')}}
