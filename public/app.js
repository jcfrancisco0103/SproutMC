...
el('pluginUpload').onclick=async()=>{const files=el('pluginFile').files;if(!files||files.length===0)return;const fd=new FormData();for(const f of files){fd.append('files',f)}fd.append('inst',activeInstance||'');const box=el('pluginUploadProgress');const bar=el('pluginUploadBar');const txt=el('pluginUploadText');box.classList.remove('hidden');bar.style.setProperty('--w','0%');txt.textContent='0%';el('pluginUpload').disabled=true;el('pluginFile').disabled=true;try{let r=await apiUpload(urlWithInst('/api/plugins/upload-multi'),fd,(pct)=>{bar.style.setProperty('--w',pct+'%');txt.textContent=pct+'%'});if(!r.ok){for(const f of files){const fd1=new FormData();fd1.append('file',f);fd1.append('inst',activeInstance||'');const r1=await apiUpload(urlWithInst('/api/plugins/upload'),fd1);if(!r1.ok)throw new Error('upload_failed')}}el('pluginFile').value='';loadPlugins()}catch(e){alert('Plugin upload failed; try single-file uploads')}finally{el('pluginUpload').disabled=false;el('pluginFile').disabled=false;bar.style.setProperty('--w','0%');txt.textContent='';box.classList.add('hidden')}}
