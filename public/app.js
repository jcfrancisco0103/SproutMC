...
async function loadFiles(p){
  ...
  entries.forEach(e=>{
    ...
    if(!e.isDir && e.name.toLowerCase().endsWith('.zip')){const uz=document.createElement('button');uz.className='btn-file';uz.textContent='Unzip';uz.onclick=async()=>{const ok=confirm('Unzip this archive here?');if(!ok)return;await apiFetch('/api/fs/unzip',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path:pathJoin(p,e.name),dest:p})});loadFiles(p)};tdA.appendChild(uz)}
