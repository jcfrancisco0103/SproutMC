...
el('uploadBtn').onclick=async()=>{const files=el('uploadFile').files;if(!files||files.length===0)return;const p=el('pathInput').value;const fd=new FormData();for(const f of files){fd.append('files',f)}fd.append('dest',p);await apiFetch('/api/fs/upload-multi',{method:'POST',body:fd});el('uploadFile').value='';loadFiles(p)}
...
el('pluginUpload').onclick=async()=>{const files=el('pluginFile').files;if(!files||files.length===0)return;const fd=new FormData();for(const f of files){fd.append('files',f)}await apiFetch('/api/plugins/upload-multi',{method:'POST',body:fd});el('pluginFile').value='';loadPlugins()}
...
el('deleteSelected').onclick=async()=>{if(selectedFiles.size===0)return;const ok=confirm(`Are you sure you want to delete ${selectedFiles.size} item(s)?`);if(!ok)return;for(const target of selectedFiles){await apiFetch('/api/fs/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path:target})})}selectedFiles.clear();loadFiles(el('pathInput').value)}
